...
(async function () {
  ...
  try {
    ...
    const PAYLOAD_FILES = ["goldhen.bin", "GoldHEN.bin", PAYLOAD_FILE, "payload.bin"]
            ...
            if (DO_PAYLOAD) {
              for (const payloadFile of PAYLOAD_FILES) {
                try {
                  ...
                  if (r.ok) {
                    payloadBlob = new Uint8Array(await r.arrayBuffer());
                    mark("PAYLOAD-FILE", payloadFile);
                    break;
                  }
                } catch (e) {
              ...
              mark(
                "PAYLOAD-BLOB",
                "file=" +
                  (payloadBlob ? "selected" : PAYLOAD_FILES.join(",")) +
                  " bytes=" +
